...
func Register(ctx context.Context, userModel *User) (*User, error) {
	...
	//var count int64 = 0
	//dao.DB.WithContext(ctx).Where("name = ?", userModel.Name).First(&userResp).Count(&count)
	//if count == 1 {
	//	return nil, errno.ExistUserError
	//}
	//WithContext(ctx)是将一个context.Context对象和数据库连接绑定，以实现在数据库操作中使用context.Context上下文传递。
	if err := dao.DB.WithContext(ctx).Where("name = ?", userModel.Name).First(&userResp).Error; err == nil {
		return nil, errno.ExistUserError
	}
...
func Login(ctx context.Context, userModel *User) (*User, error) {
	userResp := new(User)
	if err := dao.DB.WithContext(ctx).Where("name = ?", userModel.Name).
		First(&userResp).Error; err != nil {
		return nil, errno.NotExistUserError
	}

	if utils.CheckPassword(userResp.Pwd, userModel.Pwd) == false {
